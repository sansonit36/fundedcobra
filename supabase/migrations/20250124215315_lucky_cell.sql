...
BEGIN
  ...
  WITH account_stats AS (
    ...
    LEFT JOIN payout_requests pr ON pr.account_id = ta.id AND pr.user_id = ta.user_id
    WHERE ta.user_id = get_available_payouts.user_id
...
BEGIN
  ...
  FROM get_available_payouts(get_total_available_payout.user_id) ap;
END;
$$;
