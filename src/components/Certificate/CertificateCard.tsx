...
export default function CertificateCard({ certificate, compact = false, showDownload = true }: CertificateCardProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `FundedCobra-Certificate-${certificate.certificate_number}.png`;
  ...
  return (
    <div className="space-y-4">
      <div ref={cardRef} className="relative w-full rounded-none sm:rounded-sm border border-[#30363D] bg-[#0E1117] shadow-xl overflow-hidden text-left p-8 md:p-14">
        {/* Subtle background pattern */}
        <div className="absolute inset-0 opacity-[0.015]" style={{
          backgroundImage: `repeating-linear-gradient(45deg, #E6EDF3 0px, #E6EDF3 1px, transparent 1px, transparent 40px),
                            repeating-linear-gradient(-45deg, #E6EDF3 0px, #E6EDF3 1px, transparent 1px, transparent 40px)`
        }} />

        {/* Header Block */}
        <div className="relative flex flex-col md:flex-row md:items-start justify-between border-b border-[#30363D] pb-10">
          <div>
        ...
        <div className="relative mt-16 pt-10 border-t border-[#30363D] grid grid-cols-1 md:grid-cols-2 gap-10">
      ...
      {/* Download Button — outside the card so it doesn't appear in the image */}
      {showDownload && (
        <button
          onClick={handleDownload}
          disabled={downloading}
          className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-lg bg-[#21262D] hover:bg-[#30363D] border border-[#30363D] text-[#E6EDF3] text-sm font-bold uppercase tracking-widest transition-all active:scale-[0.99] disabled:opacity-50"
        >
          {downloading ? (
